...
Find the change of variables that transforms $u_{tt}+u_{tx}-2u_{xx}=0$ into the canonical form $\partial_{\xi\eta} v =0$.
...
$$
There is only one characteristic curve. So, choosing $\eta = 2A y - B x$ and $\eta = x$ (or anything else), we have that $C'=0$, which means that also $B'=0$ (as $\Delta = B^2-4AC=0$), so 
we get the canonical form
$$
A'\frac{\partial^2 v}{\partial \xi^2} =0,
$$
with the general solution $v(\xi,\eta) = F(\eta) + \xi G(\eta) = F(x)+(2Ax-By)G(x)$.
...
Let's us use the ansatz $u(x,y) = \sum_{k\in \mathbb Z}\sum_{j\in \mathbb Z} c_{k,j} e^{i k x} e^{i j y}$, where $c_{k,j}$ are the Fourier coefficients of the solution.
The elliptic PDE with a periodic source term $f(x,y) = \sum_{k\in \mathbb Z}\sum_{j\in \mathbb Z} f_{k,j} e^{i k x} e^{i j y}$ with $f_{0,0}=0$ reads
$$u_{xx}(x,y)+u_{yy}(x,y) = f(x,y)$$
We can rewrite it as
$$
\begin{align*}
&\sum_{k\in \mathbb Z}\sum_{j\in \mathbb Z} c_{k,j}(-k^2-j^2) e^{i k x} e^{i j y} = \sum_{k\in \mathbb Z}\sum_{j\in \mathbb Z} f_{k,j} e^{i k x} e^{i j y}\\
\Longrightarrow & c_{k,j} = \frac{-f_{k,j}}{k^2+j^2} \quad \text{for } (k,j)\neq (0,0), \qquad c_{0,0} \text{ is arbitrary}.
\end{align*}
$$
The last step is possible because the trigonometric functions are orthogonal, so we can directly compare only the coefficients of the two sides of the equality.
