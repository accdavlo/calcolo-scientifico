...
$$a(t,x)u_t+b(t,x)u_x=g(t,x), (t,x)\in\Omega\subset\mathbb R^2.$$
...
where $u\in \mathcal{C}^2(\Omega)$ and $A,B,C$ $\mathcal C^2$ and they do not vanish simultaneously. Let's **classify** the PDE depending on the *discriminant* 
$$
\Delta := B^2-4AC.
$$
## Definition
...
Considere the just the second order terms of the hyperbolic PDE $\Delta = B^2-4AC>0$.
...
\end{align}
$$
The equations for $\xi$ and $\eta$ are th same equations.
...
\begin{cases}
    ...
    u(t,0) = \sin(x), \quad x\in\mathbb R,\\
    u_x(t,0) = 0, \quad x\in\mathbb R.
\end{cases}
\end{align}
$$
In this case, $S=\lbrace (t,x)\in \mathbb R^2: x=0  \rbrace$ and $n=(n_t,n_x)=(0,-1)$. The general solution of the wave equation is $u(t,x) = f(x-t) + g(x+t)$, so that the initial (boundary) conditions read $f_0(x) = \sin(x)$ and $f_1(x) = 0$, so
$$
\begin{align}
    \begin{cases}
        f(-t)+g(t) = \sin(t),\\
...
We will see in the next section, how to impose properly IC and BC. 
